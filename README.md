...
`rflow` is a Linux-to-Linux keyboard and mouse sharing MVP focused on low and predictable latency.
...
- Inject a combined virtual keyboard and mouse through `/dev/uinput`.
...
Both machines need Linux. The sender must be allowed to read the selected `/dev/input/event*`
devices. The receiver must be allowed to write `/dev/uinput`. Distribution packages commonly
provide an `input` group, but group names and recommended udev policy vary.
